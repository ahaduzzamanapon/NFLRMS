@extends('layouts.app')
@section('title', 'Ammunition Overview List')
...
<div class="max-w-full space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-2xl font-bold font-serif text-slate-900 leading-tight">Ammunition Overview List</h2>
            </div>
            <p class="text-xs text-slate-500 mt-1 font-medium">Consolidated ammunition inventory &amp; quota tracking across citizen licensees &amp; registered dealers</p>
        </div>
        <div class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 flex items-center justify-between">
            <div>
                ...
                <div class="text-2xl font-extrabold text-indigo-700 tracking-tight mt-0.5">{{ number_format($totalCount) }}</div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Citizen Quota Allocation</span>
                <div class="text-2xl font-extrabold text-blue-600 tracking-tight mt-0.5">{{ number_format($citizenCount) }}</div>
                <span class="text-[10px] text-slate-500 font-medium">Annual licensed allowances</span>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold uppercase text-slate-400 tracking-wider">Dealer Commercial Stock</span>
                <div class="text-2xl font-extrabold text-purple-600 tracking-tight mt-0.5">{{ number_format($dealerCount) }}</div>
                <span class="text-[10px] text-slate-500 font-medium">Vault &amp; inventory store</span>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-3">
            <form method="GET" action="{{ route('overview.ammunition') }}" class="w-full sm:w-auto flex flex-col sm:flex-row items-center gap-2">
                ...
                @if(request('search') || request('user_type'))
                    <a href="{{ route('overview.ammunition') }}" class="text-xs text-rose-600 hover:text-rose-700 font-semibold px-2 py-1">Clear</a>
                @endif
            </form>
            ...
            <table class="w-full text-left border-collapse min-w-[750px]">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-[10px] font-bold uppercase text-slate-500 tracking-wider">
                        ...
                        <th class="p-3.5">Ammunition Type</th>
                        <th class="p-3.5">Bore / Calibre</th>
                        <th class="p-3.5 text-center">Quantity (Rounds)</th>
                ...
                <tbody class="text-xs divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="p-3.5 text-center">
                            <span class="inline-block px-2.5 py-0.5 rounded-lg bg-indigo-50 text-indigo-800 font-extrabold font-mono text-xs border border-indigo-100">
                    ...
                    @empty
                    <tr>
                        <td colspan="8" class="p-8 text-center text-slate-400 font-medium">No ammunition records found matching your criteria.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="p-3.5 border-t border-slate-100 bg-slate-50/50 text-[11px] text-slate-500 text-center font-medium">
            Total Ammunition Quantity across list: <strong class="text-indigo-700 font-bold font-mono">{{ number_format($totalCount) }}</strong> rounds
        </div>
    </div>
</div>

<!-- Details Modal -->
<div id="ammo-detail-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-lg w-full overflow-hidden transform transition-all">
        ...
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h3 class="text-sm font-bold text-slate-900">Ammunition Record Details</h3>
        ...
        <div class="p-6 space-y-4 text-xs">
            ...
            <div class="space-y-2">
                <h4 class="text-[11px] font-bold uppercase text-slate-400 tracking-wider">Ammunition Stock Details</h4>
                <div class="grid grid-cols-2 gap-2.5">
                    <div class="p-2.5 bg-white border border-slate-200/80 rounded-xl">
                        <span class="text-[10px] text-slate-400 block">Ammo Specification</span>
                    ...
                    <div class="p-2.5 bg-white border border-slate-200/80 rounded-xl">
                        <span class="text-[10px] text-slate-400 block">Quantity Held / Stock</span>
                        <strong class="text-indigo-700 font-mono text-sm" id="modal-qty"></strong>
                    </div>
                    <div class="p-2.5 bg-white border border-slate-200/80 rounded-xl">
            ...
            <div class="space-y-2">
                <h4 class="text-[11px] font-bold uppercase text-slate-400 tracking-wider">Licensing Authority &amp; Verification</h4>
...
function openDetailModal(item) {
    ...
    document.getElementById('modal-qty').innerText = item.quantity + ' rounds';
    ...
    document.getElementById('ammo-detail-modal').classList.remove('hidden');
}

function closeDetailModal() {
    document.getElementById('ammo-detail-modal').classList.add('hidden');
}
