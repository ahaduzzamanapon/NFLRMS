...
<div class="max-w-3xl space-y-5">
    ...
    <!-- Page Header -->
    <div>
        <h2 class="text-xl font-black font-serif text-slate-900">Custom Comment</h2>
    ...
    <form action="{{ isset($customComment) ? route('custom_comment.update', $customComment) : route('custom_comment.store') }}" method="POST"
        ...
        <div class="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <span class="text-[10px] font-extrabold uppercase text-slate-900 font-black tracking-widest">
                {{ isset($customComment) ? '✏️ Edit Custom Comment' : 'Create New Custom Comment' }}
            </span>
            @if(isset($customComment))
                <a href="{{ route('custom_comment.index') }}" class="text-[10px] font-bold text-slate-400 hover:text-gov-green transition-colors">← Cancel Edit</a>
            @endif
        </div>
        ...
        <div class="p-5 space-y-4">
            <div>
                <label for="title" class="block text-[10px] font-extrabold uppercase text-slate-900 mb-1.5">Title</label>
            ...
            <div>
                <label for="comment" class="block text-[10px] font-extrabold uppercase text-slate-900 mb-1.5">Description</label>
            ...
            <div class="flex justify-end">
                <button type="submit"
                        class="px-5 py-2.5 {{ isset($customComment) ? 'bg-amber-500 hover:bg-amber-600' : 'bg-gov-green hover:bg-gov-light' }} text-white text-xs font-black rounded-lg transition-colors shadow-sm">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-5 py-3 border-b border-slate-100 bg-slate-50">
            <span class="text-[10px] font-extrabold uppercase text-slate-900 font-black tracking-widest">My Saved Comments</span>
            <span class="ml-2 px-2 py-0.5 rounded-full bg-slate-200 text-slate-600 text-[9px] font-black">{{ $comments->count() }}</span>
        </div>
        <div class="p-5">
            @forelse($comments as $comment)
                <div class="flex items-start justify-between p-3.5 rounded-lg border border-slate-200 bg-slate-50/70 {{ !$loop->first ? 'mt-3' : '' }}">
                    <div class="space-y-1 min-w-0 pr-3">
                        <span class="font-bold text-slate-900 text-sm block">{{ $comment->title }}</span>
                    ...
                    <div class="flex items-center space-x-2 flex-shrink-0">
                        <a href="{{ route('custom_comment.edit', $comment) }}"
                           class="px-2.5 py-1.5 rounded bg-blue-50 hover:bg-blue-100 text-blue-600 text-[10px] font-bold border border-blue-200 transition-colors">
                            ✏️ Edit
                        </a>
                        <form action="{{ route('custom_comment.destroy', $comment) }}" method="POST"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="px-2.5 py-1.5 rounded bg-rose-50 hover:bg-rose-100 text-rose-600 text-[10px] font-bold border border-rose-200 transition-colors">
                                🗑 Delete
                            </button>
                        </form>
                    </div>
            ...
            @empty
                <div class="text-center py-8 space-y-2">
                    ...
                    <p class="text-xs text-slate-400 font-bold">No custom comments yet.</p>
