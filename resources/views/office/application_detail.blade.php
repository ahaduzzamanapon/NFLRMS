...
<div class="max-w-7xl space-y-4">
                ...
                @foreach($pipeline as $key => $step)
                    ...
                    <div class="flex-1 relative">
                        <div class="flex flex-col items-center">
                            <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-black border-2 transition-all
    ...
    <div class="flex flex-wrap items-center gap-1.5 bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
        <button type="button" data-tab="overview" onclick="switchDetailTab('overview')"
                class="detail-tab flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-[10px] font-extrabold uppercase transition-all focus:outline-none bg-gov-green text-white shadow-sm">
            <span>👤</span><span>Overview</span>
        </button>
        <button type="button" data-tab="documents" onclick="switchDetailTab('documents')"
                class="detail-tab flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-[10px] font-extrabold uppercase transition-all focus:outline-none text-slate-500 hover:bg-slate-50">
            <span>📎</span><span>Documents</span>
            <span class="px-1.5 py-0.5 rounded-full text-[8px] font-black {{ $uploadedCount === count($standardDocList) ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">{{ $uploadedCount }}/{{ count($standardDocList) }}</span>
        </button>
        <button type="button" data-tab="timeline" onclick="switchDetailTab('timeline')"
                class="detail-tab flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-[10px] font-extrabold uppercase transition-all focus:outline-none text-slate-500 hover:bg-slate-50">
            <span>🕐</span><span>Timeline</span>
            <span class="px-1.5 py-0.5 rounded-full text-[8px] font-black bg-slate-100 text-slate-500">{{ $application->logs->count() }}</span>
        </button>
        ...
        <button type="button" data-tab="vetting" onclick="switchDetailTab('vetting')"
            ...
            <span class="px-1.5 py-0.5 rounded-full text-[8px] font-black bg-slate-100 text-slate-500">{{ $application->vettings->count() }}</span>
        </button>
        @endif
    </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="detail-panel" id="panel-overview">
                    <div class="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                        <span class="text-[10px] font-extrabold uppercase text-slate-500 tracking-widest">👤 Applicant Summary</span>
                        <button type="button" onclick="openApplicantModal()"
                                class="px-3 py-1.5 rounded-lg bg-gov-green hover:bg-gov-light text-white text-[10px] font-black transition-colors shadow-sm flex items-center space-x-1">
                            <span>🔍</span><span>View Full Details</span>
                        </button>
                    </div>
                    <div class="p-5">
                        <div class="flex items-center space-x-4">
                            <div class="w-14 h-14 rounded-full bg-gov-green/10 border-2 border-gov-green/30 flex items-center justify-center text-gov-green font-black text-xl flex-shrink-0">
                                {{ strtoupper(substr($application->user->name, 0, 1)) }}
                            </div>
                            <div class="min-w-0">
                                <div class="font-bold text-slate-900 text-sm truncate">{{ $application->user->name }}
                                    @if($application->user->name_bn) <span class="text-[11px] text-slate-500 font-semibold">({{ $application->user->name_bn }})</span> @endif
                                </div>
                                <div class="text-[10px] text-slate-500 font-semibold mt-0.5">
                                    NID: {{ $application->applicant_details['nid'] ?? $application->user->nid ?? 'N/A' }}
                                </div>
                                <div class="flex items-center space-x-3 mt-1.5 text-[10px] text-slate-500 font-semibold">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-100">
                            <div class="p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Weapon</span>
                                <span class="text-xs font-bold text-slate-800">{{ $application->firearm_details['weapon_type'] ?? 'N/A' }}</span>
                            </div>
                            <div class="p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Bore</span>
                                <span class="text-xs font-bold text-slate-800">{{ $application->firearm_details['bore'] ?? 'N/A' }}</span>
                            </div>
                            <div class="p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Occupation</span>
                                <span class="text-xs font-bold text-slate-800 truncate">{{ $application->applicant_details['occupation'] ?? $application->user->occupation ?? 'N/A' }}</span>
                            </div>
                            <div class="p-2.5 rounded-lg bg-slate-50 border border-slate-100">
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Income</span>
                        ...
                        <div class="mt-4 p-3 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100 border border-slate-200">
                            <span class="text-[9px] font-extrabold uppercase text-slate-400 tracking-widest block mb-1">🧭 Routing Rule</span>
                            <p class="text-xs text-slate-600 font-semibold">
                                @if(in_array($application->firearm_details['weapon_type'] ?? '', ['Pistol','Revolver']))
                                    <span class="text-rose-600 font-black">Handgun case</span> → MoHA approval required.
                                @else
                                    <span class="text-emerald-700 font-black">Long-gun case</span> → DC direct approval.
                                @endif
                            </p>
                        </div>
                        ...
                        <div class="grid grid-cols-4 gap-3 mt-4">
                            <div class="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                                <span class="text-lg font-black text-gov-green block">{{ $application->logs->count() }}</span>
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-wider">Events</span>
                            </div>
                            <div class="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                                <span class="text-lg font-black text-amber-600 block">{{ $application->vettings->count() }}</span>
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-wider">Vettings</span>
                            </div>
                            <div class="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                                <span class="text-lg font-black text-emerald-600 block">{{ $uploadedCount }}</span>
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-wider">Docs</span>
                            </div>
                            <div class="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                                <span class="text-lg font-black text-blue-600 block">{{ $application->created_at->format('d M') }}</span>
                                <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-wider">Filed</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="detail-panel hidden" id="panel-documents">
                    <div class="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                        <span class="text-[10px] font-extrabold uppercase text-slate-500 tracking-widest">📎 Attached Documents</span>
                        @if($hasUploadedDocs)
                            <span class="text-[9px] font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full">✓ Uploaded & Verified</span>
                        @else
                            <span class="text-[9px] font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded-full">⚠️ Pending Upload</span>
                        @endif
                    </div>
                    <div class="p-5">
                            ...
                            <div class="p-3 rounded-xl border transition-all group
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-2.5 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <span class="font-bold text-slate-800 block text-[11px] leading-tight truncate">{{ $doc['name'] }}</span>
                                            @if(!$doc['is_uploaded'])
                                                <span class="text-[9px] text-rose-600 font-bold">Not uploaded</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex items-center space-x-1.5 flex-shrink-0">
                                        @if($doc['is_uploaded'])
                                            ...
                                            <button type="button" onclick="openOfficeDocumentViewer('{{ addslashes($doc['name']) }}', '{{ $doc['file_name'] }}', '{{ $doc['file_size'] }}', true, '{{ $doc['key'] }}')"
                                                    class="px-2 py-1 rounded-lg bg-gov-green hover:bg-gov-light text-white text-[10px] font-bold transition-all shadow-sm">
                                                👁️ View
                                            </button>
                                        @else
                                            ...
                                            <button type="button" onclick="openOfficeDocumentViewer('{{ addslashes($doc['name']) }}', 'No file uploaded', '0 KB', false, '{{ $doc['key'] }}')"
                                                    class="px-2 py-1 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-700 text-[10px] font-bold transition-all">
                                                👁️ Inspect
                                            </button>
                                        @endif
                                    </div>
                ...
                <div class="detail-panel hidden" id="panel-timeline">
                    <div class="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                        <span class="text-[10px] font-extrabold uppercase text-slate-500 tracking-widest">🕐 Case Timeline</span>
                        <span class="text-[9px] font-bold text-slate-400">{{ $application->logs->count() }} events</span>
                    </div>
                    <div class="p-5">
                        ...
                        <div class="relative flex space-x-4 pb-6 last:pb-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span class="text-[9px] text-slate-400 font-bold flex-shrink-0">{{ $log->created_at->format('d M Y · h:i A') }}</span>
                                </div>
                                <p class="text-xs text-slate-700 font-medium leading-relaxed mt-1">{{ $log->remarks }}</p>
                                @if($log->actor)
                                <div class="flex items-center space-x-1.5 mt-1.5">
                                    <span class="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-[8px] font-black text-slate-600 flex-shrink-0">
                                        {{ strtoupper(substr($log->actor->name, 0, 1)) }}
                                    </span>
                                    <span class="text-[9px] text-slate-500 font-semibold">by {{ $log->actor->name }}</span>
                                </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="text-center py-8 space-y-2">
                            ...
                            <p class="text-xs text-slate-400 font-bold">No timeline entries yet.</p>
                        </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="detail-panel hidden" id="panel-vetting">
                    <div class="px-5 py-3 border-b border-slate-100 bg-slate-50">
                        <span class="text-[10px] font-extrabold uppercase text-slate-500 tracking-widest">🛡️ Security Vetting Reports</span>
                    </div>
                    <div class="p-5">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden lg:sticky lg:top-4">
                ...
                <div class="p-5 space-y-3">
                    <form action="{{ $actionRoute }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            <label for="custom_comment_select" class="block text-[9px] font-extrabold uppercase text-slate-400 tracking-widest mb-1.5">💬 Quick Fill</label>
                        ...
                        <div>
                            <label for="remarks" class="block text-[9px] font-extrabold uppercase text-slate-400 tracking-widest mb-1.5">📝 Remarks</label>
    ...
    <div class="bg-white rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden border border-slate-200 max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 py-4 bg-gov-green text-white flex items-center justify-between flex-shrink-0">
            <div class="flex items-center space-x-2.5">
                ...
                <div>
                    <h3 class="text-sm font-black uppercase tracking-wider">Applicant Full Details</h3>
                    <p class="text-[10px] text-white/70 font-semibold">Case {{ $application->application_number }}</p>
                </div>
            </div>
            <button type="button" onclick="closeApplicantModal()" class="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 text-white font-bold text-sm flex items-center justify-center transition-colors">
                ✕
            </button>
        </div>
        ...
        <div class="p-6 overflow-y-auto flex-grow">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                <div class="sm:col-span-2">
                    <span class="text-[9px] font-extrabold uppercase text-gov-green tracking-widest block mb-2 border-b border-slate-100 pb-1.5">🪪 Identity</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Full Name</span>
                    <span class="font-bold text-slate-900">{{ $application->user->name }}</span>
                    @if($application->user->name_bn) <span class="text-[11px] text-slate-500 font-semibold block">({{ $application->user->name_bn }})</span> @endif
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">National ID (NID)</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['nid'] ?? $application->user->nid ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Date of Birth</span>
                ...
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Phone</span>
                    <span class="font-bold text-slate-900">{{ $application->user->phone ?? $application->applicant_details['phone'] ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Marital Status</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['marital_status'] ?? $application->user->marital_status ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Nationality</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['nationality'] ?? $application->user->nationality ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Religion</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['religion'] ?? $application->user->religion ?? 'N/A' }}</span>
                </div>

                <!-- Family -->
                <div class="sm:col-span-2 mt-2">
                    <span class="text-[9px] font-extrabold uppercase text-gov-green tracking-widest block mb-2 border-b border-slate-100 pb-1.5">👨‍👩‍👧 Family</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Father's Name</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['father_name'] ?? $application->user->father_name ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Mother's Name</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['mother_name'] ?? $application->user->mother_name ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Spouse Name</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['spouse_name'] ?? $application->user->spouse_name ?? 'N/A' }}</span>
                </div>

                <!-- Address -->
                <div class="sm:col-span-2 mt-2">
                    <span class="text-[9px] font-extrabold uppercase text-gov-green tracking-widest block mb-2 border-b border-slate-100 pb-1.5">📍 Address</span>
                </div>
                <div class="sm:col-span-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Present Address</span>
                ...
                <div class="sm:col-span-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Permanent Address</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['permanent_address'] ?? $application->user->permanent_address ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">District</span>
                    <span class="font-bold text-slate-900">{{ $application->user->district->name ?? $application->district->name ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Upazila / Thana</span>
                    <span class="font-bold text-slate-900">{{ $application->user->upazila->name ?? $application->upazila->name ?? 'N/A' }}</span>
                </div>

                <!-- Occupation & Income -->
                <div class="sm:col-span-2 mt-2">
                    <span class="text-[9px] font-extrabold uppercase text-gov-green tracking-widest block mb-2 border-b border-slate-100 pb-1.5">💼 Occupation & Income</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Occupation</span>
                ...
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Employer Address</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['employer_address'] ?? $application->user->employer_address ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Annual Income</span>
                    <span class="font-bold text-slate-900">
                ...
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">TIN Number</span>
                    <span class="font-bold text-slate-900">{{ $application->applicant_details['tin_number'] ?? $application->user->tin_number ?? 'N/A' }}</span>
                </div>

                <!-- Firearm Details -->
                <div class="sm:col-span-2 mt-2">
                    <span class="text-[9px] font-extrabold uppercase text-gov-green tracking-widest block mb-2 border-b border-slate-100 pb-1.5">🔫 Firearm Details</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Weapon Type</span>
                    <span class="font-bold text-slate-900">{{ $application->firearm_details['weapon_type'] ?? 'N/A' }}</span>
                </div>
                <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Bore / Calibre</span>
                    <span class="font-bold text-slate-900">{{ $application->firearm_details['bore'] ?? 'N/A' }}</span>
                </div>
                <div class="sm:col-span-2 p-3 bg-slate-50 rounded-lg border border-slate-100">
                    <span class="text-[8px] font-extrabold uppercase text-slate-400 tracking-widest block">Purpose</span>
                ...
                <div class="sm:col-span-2 p-3 bg-emerald-50 rounded-lg border border-emerald-200">
                    <span class="text-[8px] font-extrabold uppercase text-emerald-600 tracking-widest block">Licensed Arms Dealer / Sourcing Store</span>
        ...
        <div class="px-6 py-3 bg-slate-50 border-t border-slate-200 flex items-center justify-between flex-shrink-0">
            <span class="text-[10px] text-slate-400 font-semibold">NFLRMS Official Case File</span>
            <button type="button" onclick="closeApplicantModal()" class="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold text-xs rounded-lg transition-colors">
                Close
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div class="px-5 py-4 bg-gov-green text-white flex items-center justify-between">
            <div class="flex items-center space-x-2.5">
                <span class="text-xl">📄</span>
                <div>
                    <h3 id="officeModalDocTitle" class="text-xs font-black uppercase tracking-wider">Document Title</h3>
                    <p id="officeModalDocMeta" class="text-[10px] text-white/70 font-semibold">filename.pdf &bull; 1.5 MB</p>
                </div>
            </div>
    ...
    function switchDetailTab(tabName) {
        ...
        document.querySelectorAll('.detail-tab').forEach(btn => {
            ...
            btn.className = isActive
                ? 'detail-tab flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-[10px] font-extrabold uppercase transition-all focus:outline-none bg-gov-green text-white shadow-sm'
                : 'detail-tab flex items-center space-x-1.5 px-3.5 py-2 rounded-lg text-[10px] font-extrabold uppercase transition-all focus:outline-none text-slate-500 hover:bg-slate-50';
        });
    }
